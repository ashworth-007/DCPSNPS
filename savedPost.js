  ...
  async function fetchSavedPosts() {
    ...
    try {
      ...
      if (!savedPostsDoc.exists()) {
        document.getElementById('posts-container').innerHTML = '<p>No saved posts available.</p>';
        return;
      }
      ...
      // Fetch and render each saved post
      for (const postId in savedPosts) {
        const postData = savedPosts[postId];
        await renderPost(postId, postData);
      }
    } catch (error) {
      console.error("Error fetching saved posts:", error);
      document.getElementById('posts-container').innerHTML = '<p>Error fetching saved posts. Check console for details.</p>';
    }
  }
